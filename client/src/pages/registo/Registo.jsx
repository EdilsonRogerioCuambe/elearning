...
const Registo = () => {
  ...
  const onSubmit = async (data) => {
    setLoading(true);
    const { nome, endereco, email, senha, dataNascimento, cpf, sexo, telefone } = data;
    ...
    try {
      const response = await axios.post('http://localhost:5000/api/estudantes',
        formData,
        {
          headers: {
      ...
      localStorage.setItem('tipo', 'estudantes');
      setLoading(false);
      navigate('/');
    } catch (error) {
    ...
    <Layout>
      <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 font-mono mt-32">
        <form onSubmit={handleSubmit(onSubmit)} className="border-2 border-yellow-400 shadow-md rounded px-8 pt-6 pb-8 mb-4" encType='multipart/form-data'>
          <h2 className="text-2xl mb-4">Registro Estudante</h2>
